...
export class FileDownload {
	...
	// 创建hyMusic下文件目录
	createPath() {
		let MainFileName = plus.os.name === 'iOS' ? this.downPath : getPath({ type: 'music' })
	...
	async indexDataSave(dataList, type, index, callback) {
		// 如果数据为空则不做任何处理
		if (dataList === undefined || dataList.length === 0 || !dataList) return;
				...
				if (status === 200 && download.state === 4) {
					...
					if (isFile) {
						...
						if (fileType === 'music' || (fileType === 'image' && this.isMove)) {
							...
							values = {
								url: (type === 'url' && this.urlType === 0) ? this.platform === 'iOS' ? download.filename : plus.io.convertLocalFileSystemURL(this.temp) : info.url, // 歌曲所在本地地址
								hq_url: (type === 'url' && this.urlType === 1) ? this.platform === 'iOS' ? download.filename : plus.io.convertLocalFileSystemURL(this.temp) : info.hq_url,
								sq_url: (type === 'url' && this.urlType === 2) ? this.platform === 'iOS' ? download.filename : plus.io.convertLocalFileSystemURL(this.temp) : info.sq_url,
								avatar: type === 'avatar' ? this.platform === 'iOS' ? download.filename : plus.io.convertLocalFileSystemURL(this.temp) : info.avatar, // 歌曲头像本地地址
								background: type === 'background' ? this.platform === 'iOS' ? download.filename : plus.io.convertLocalFileSystemURL(this.temp) : info.background, // 歌曲背景图片本地地址
							}
						}
						// 下载首页图片
