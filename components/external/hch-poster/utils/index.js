  ...
  return new Promise((resolve, reject) => {
    ...
    if (url) {
      if (!url.includes('base64')) {
        ...
        let extName = url.match(/data\:\S+\/(\S+);/)
        ...
        fsm.writeFile({
          filePath,
          data: url.replace(/^data:\S+\/\S+;base64,/, ''), //替换前缀为空
          encoding: 'base64',
          success:(res)=>{
 ...
 */
export function drawTextReturnH(ctx, text, x, y, maxWidth = 375, fontSize = 14, color = '#000', lineHeight = 30,
  textAlign = 'left') {
  ...
  for (let n = 0; n < arrText.length; n++) {
    ...
    if (textAlign) {
      ...
      switch (textAlign) {
        ...
        case 'right':
          x = (getSystem().w - maxWidth) / 2 + maxWidth - 11
          break;

        default: // 左对齐
          x = (getSystem().w - maxWidth) / 2 + 11
          break;
      }
    }
    ...
    let testWidth = metrics.width + 72;
...
export function getSystem() {
  ...
  let scale = system.screenWidth / 375 //按照苹果留 375*667比例 其他型号手机等比例缩放 显示
  return {
    w: system.screenWidth,
    h: system.screenHeight,
    scale: scale
  }
}
