    ...
    <view :class="{ h5Top: hasH5Head && $isH5, fixed: position === 'fixed', noNativeHead }" :style="{ top }"
      ...
      <view v-show="mode === 'button'" class="tf-main white-bg">
        ...
        <view v-for="(item, index) in list" :class="{ active: index === currentIndex }" :key="index"
          ...
          <!--日期-->
         <picker v-if="item.type === 'date' && item.subType === 'undefined'" key="date" :end="String(end)" mode="date" class="tf-btns"
  ...
  export default {
    ...
    watch: {
      ...
      options(v) {
        this.list = v
        console.log('v=',v)
        // console.log('watch options====',this.list)
      },
      showMask(v) {
        this.$emit('show', v)
      },
    ...
    methods: {
      ...
      onChildClick(parentIndex, subIndex, sub) {
        ...
        child[subIndex] = sub
        this.list = arr
        // console.log('onChildClick======list==',this.list)
      ...
      onDateChange({
        ...
        this.currentIndex = index
        this.list = arr
        // console.log('onDateChange======list===',this.list)
